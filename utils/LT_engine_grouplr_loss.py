...
class MultiLabelEngine():
    ...
    def validate(self, models, clip_model,val_loader, criterion):
        ...
        for i, (inputData, target, semantic) in enumerate(val_loader):
            ...
            with torch.no_grad():
                with autocast():
                    # regular model
                    _, _,regular_cls_output1, regular_cls_output2,_,regular_dist_feat = regular_model(inputData.cuda())
                    # regular_cls_output, _,regular_dist_feat = regular_model(inputData.cuda())
                    regular_cls_output1 = regular_cls_output1.float()
                    regular_cls_output2 = regular_cls_output2.float()
                    regular_cls_output = (regular_cls_output1 + regular_cls_output2)/2

                    regular_overall_loss = (criterion[0](regular_cls_output1, target) + criterion[0](regular_cls_output2, target))/2
                    ...
                    # regular_dist_loss = F.SmoothL1Loss(regular_dist_feat.float(), regular_tea_dist_feat.float())

                    regular_loss = regular_overall_loss + regular_dist_loss * self.ratio
            # ema model
            with torch.no_grad():
                _, _,ema_cls_output1,ema_cls_output2, _,ema_dist_feat = ema_model.module(inputData.cuda())
                # ema_cls_output, _,ema_dist_feat = ema_model.module(inputData.cuda())
                ema_cls_output1 = ema_cls_output1.float()
                ema_cls_output2 = ema_cls_output2.float()
                ema_cls_output = (ema_cls_output1 + ema_cls_output2)/2
                ema_overall_loss = (criterion[0](ema_cls_output1, target) +  criterion[0](ema_cls_output2, target))/2
                ...
                # ema_dist_loss = F.SmoothL1Loss(ema_dist_feat.float(), ema_tea_dist_feat.float())
                ema_loss = ema_overall_loss + ema_dist_loss * self.ratio
    ...
    def train(self, models,clip_model,train_loader, criterion, optimizer, scheduler, scaler, epoch):
        ...
        for i, (inputData, target, semantic) in enumerate(train_loader):
            ...
            with autocast():  # mixed precision
                _, _,cls_output1,cls_output2, _,dist_feat = regular_model(inputData)
                # cls_output, _,dist_feat = regular_model(inputData)
                cls_output1 = cls_output1.float()
                cls_output2 = cls_output2.float()
                cls_output = (cls_output1 + cls_output2)/2

                cls_loss = (criterion[0](cls_output1, target) + criterion[0](cls_output2, target))/2
                ...
                # dist_loss = F.SmoothL1Loss(dist_feat.float(), tea_dist_feat)

            regular_loss = cls_loss + dist_loss * self.ratio
            ...
            with torch.no_grad():
                ...
                _, _,ema_cls_output1,ema_cls_output2, _, ema_dist_feat = ema_model.module(inputData.float())
                # ema_cls_output, _, ema_dist_feat = ema_model.module(inputData.float())
                ema_cls_output1 = ema_cls_output1.float()
                ema_cls_output2 = ema_cls_output2.float()
                ema_cls_output  = (ema_cls_output1 + ema_cls_output2)/2

            ema_cls_loss = (criterion[0](ema_cls_output1, target) + criterion[0](ema_cls_output2, target))/2
            ...
            ema_loss = ema_cls_loss + ema_dist_loss * self.ratio
    ...
    def save_checkpoint(self, models, regular_ap, regular_map, ema_ap, ema_map, epoch, model_name):
        ...
        if not os.path.exists('MKT_LT_checkpoint/'):
            os.mkdir('MKT_LT_checkpoint/')
        ...
        if ema_map > self.highest_ema_map:
            ...
            if self.highest_ema_model is not None:
                os.remove(os.path.join('/data2/yanjiexuan/checkpoints/RC-Tran/MKT_LT_checkpoint/', self.highest_ema_model))
            ...
            torch.save({
                ...
                'model': 'ema',
                'state_dict': ema_model.module.state_dict(),
            }, os.path.join('/data2/yanjiexuan/checkpoints/RC-Tran/MKT_LT_checkpoint/', self.highest_ema_model))
            if self.dataset == 'coco-lt' or self.dataset == 'voc-lt':
