...
class MultiLabelEngine():
    def __init__(self, args):
        ...
        self.num_heatmaps = args.get('num_heatmaps', 10)
        self.heatmap_save_dir = args.get('heatmap_save_dir', 'figures/heatmaps')
        self._heatmap_saved = 0
    ...
    def validate(self, model, val_loader):
        ...
        for i, (inputData, target, semantic) in enumerate(val_loader):
            ...
            with torch.no_grad():
                with autocast():
                    # 支持 4 种模型输出: 1-Tensor(clip/coop) / 3-tuple(clip_vit) / 5-tuple(clip_vit_dual)
                    out = regular_model(inputData.cuda())
                    if isinstance(out, torch.Tensor):
                        regular_cls_output = out
                    elif len(out) == 3:
                        regular_cls_output = out[0]
                    else:
                        _, _, regular_cls_output, _, _ = out
            ...
            if self._heatmap_saved < self.num_heatmaps and hasattr(regular_model, 'get_spatial_attention'):
                with torch.no_grad():
                    att = regular_model.get_spatial_attention(inputData.cuda())
                from utils.heatmap_utils import save_heatmap_overlay_resize
                for b in range(att.shape[0]):
                    if self._heatmap_saved >= self.num_heatmaps:
                        break
                    path = os.path.join(self.heatmap_save_dir, 'heatmap_{}.png'.format(self._heatmap_saved))
                    save_heatmap_overlay_resize(
                        inputData[b:b+1], att[b], path,
                        heatmap_alpha=0.5, image_size=224
                    )
                    self._heatmap_saved += 1
