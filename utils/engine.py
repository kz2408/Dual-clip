...
class MultiLabelEngine():
    ...
    def learning(self, models, train_loader, val_loader, criterion, optimizer, scheduler):
        ...
        if self.evaluation:
            ...
            for epoch in range(self.start_epoch+1, self.max_epoch):
                ...
                self.meter_reset()
                self.validate(models, val_loader, criterion)
                regular_map, ema_map = self.meter_print()
                self.save_checkpoint(models, regular_map, ema_map, epoch)
                models[0].train()
    ...
    def validate(self, models, val_loader, criterion):
        ...
        for i, (inputData, target, semantic) in enumerate(val_loader):
            # compute output
            target = target.cuda()
            semantic = semantic[0].cuda()
            # regular model
            with torch.no_grad():
                with autocast():
                    # regular model
                    regular_cls_output, regular_semantic_output = regular_model(inputData.cuda(), semantic.cuda())
            ...
            # ema model
            with torch.no_grad():
                ema_cls_output, ema_semantic_output = ema_model.module(inputData.cuda(), semantic.cuda())
    ...
    def train(self, models, train_loader, criterion, optimizer, scheduler, scaler, epoch):
        ...
        for i, (inputData, target, semantic) in enumerate(train_loader):
            ...
            # store information
            train_loader.set_postfix(cls=cls_loss.item(), cos = cos_loss.item(), e_cls=ema_cls_loss.item(), e_cos = ema_cos_loss.item())
    ...
    def save_checkpoint(self, models, regular_map, ema_map, epoch):
        # save current model
        regular_model = models[0]
        ema_model = models[1]
        ...
        if regular_map > self.highest_regular_map:
            ...
            self.highest_regular_model = 'regular_{}_best_{score:.3f}.ckpt'.format(model_name, score=regular_map)
            torch.save({
                'epoch': epoch,
                'score': regular_map,
                'model': 'regular',
        ...
        if ema_map > self.highest_ema_map:
            ...
            if self.highest_ema_model is not None:
                os.remove(os.path.join('checkpoint/', self.highest_ema_model))
            self.highest_ema_model = 'ema_{}_best_{score:.3f}.ckpt'.format(model_name, score=ema_map)
            torch.save({
                'epoch': epoch,
                'score': ema_map,
                'model': 'ema',
                'state_dict': ema_model.module.state_dict(),
            }, os.path.join('checkpoint/', self.highest_ema_model))
