...
class AveragePrecisionMeter(object):
    ...
    def evaluation(self, scores_, targets_):
        ...
        sum_ng = np.sum(Ng)
        OR = np.sum(Nc) / sum_ng if sum_ng > 0 else 0.0
        denom_of1 = OP + OR
        OF1 = (2 * OP * OR) / denom_of1 if denom_of1 > 0 else 0.0

        CP = np.sum(Nc / Np) / n_class
        # 仅对 Ng>0 的类计算 Nc/Ng，避免除零
        CR = np.sum(np.where(Ng > 0, Nc / Ng, 0.0)) / n_class
        denom_cf1 = CP + CR
        CF1 = (2 * CP * CR) / denom_cf1 if denom_cf1 > 0 else 0.0
