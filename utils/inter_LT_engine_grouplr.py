    ...
    def validate(self, models, clip_model,val_loader, criterion):
        ...
        for i, (inputData, target, semantic) in enumerate(val_loader):
            ...
            with torch.no_grad():
                with autocast():
                    # regular model
                    regular_text_features,regular_template_features,regular_cls_output, _,regular_dist_feat = regular_model(inputData.cuda())
                    ...
                    regular_dist_loss1 = F.l1_loss(regular_dist_feat.float(), regular_tea_dist_feat.float())
                    regular_dist_loss2 = F.l1_loss(regular_text_features.float(), regular_template_features.float())

                    regular_loss = regular_overall_loss + (regular_dist_loss1 + regular_dist_loss2)/2
            # ema model
            with torch.no_grad():
                ema_text_features,ema_template_features,ema_cls_output, _,ema_dist_feat = ema_model.module(inputData.cuda())
                ...
                ema_dist_loss1 = F.l1_loss(ema_dist_feat.float(), ema_tea_dist_feat.float())
                ema_dist_loss2 = F.l1_loss(ema_text_features.float(), ema_template_features.float())
                ema_loss = ema_overall_loss + (ema_dist_loss1 + ema_dist_loss2)/2

            val_loader.set_postfix(cls=regular_overall_loss.item(), cos = regular_dist_loss1.item(), e_cls=ema_overall_loss.item(), e_cos = ema_dist_loss1.item())
    ...
    def train(self, models,clip_model,train_loader, criterion, optimizer, scheduler, scaler, epoch):
        ...
        for i, (inputData, target, semantic) in enumerate(train_loader):
            ...
            with autocast():  # mixed precision
                text_feature,template_feature,cls_output, _,dist_feat = regular_model(inputData)
                ...
                dist_loss1 = F.l1_loss(dist_feat.float(), tea_dist_feat)
                dist_loss2 = F.l1_loss(text_feature.float(), template_feature.float())

            regular_loss = cls_loss + (dist_loss1 + dist_loss2)/2
            ...
            with torch.no_grad():
                ...
                ema_text_feature, ema_template_feature,ema_cls_output, _, ema_dist_feat = ema_model.module(inputData.float())
                ema_cls_output = ema_cls_output.float()
            ...
            ema_dist_loss1 = F.l1_loss(ema_dist_feat.float(), ema_tea_dist_feat.float())
            ema_dist_loss2 = F.l1_loss(ema_text_feature.float(), ema_template_feature.float())

            ema_loss = ema_cls_loss + (ema_dist_loss1 + ema_dist_loss2)/2
            
            # store information
            train_loader.set_postfix(cls=cls_loss.item(), cos = dist_loss1.item(), e_cls=ema_cls_loss.item(), e_cos = ema_dist_loss1.item(), lr=temp_lr)
