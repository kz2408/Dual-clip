...
def read_object_labels_csv(file, header=True):
    ...
    return images, True if num_categories == 65 else False
...
class NUSWIDEClassification_fsl(data.Dataset):
    has_selected = []
    def __init__(self, root, file_csv, transform=None, target_transform=None, word_emb_file=None):
        ...
        random.seed(0)
        self.novel_cls_ind = random.sample(range(len(object_categories)), 16)
